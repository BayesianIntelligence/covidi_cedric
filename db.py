...
def update_tables(db_file, json_data):
    ...
    try:
        ...
        for item in json_data:
            ...
            #insert time series records into SQL
            for inner_item in item.get('TimeSeries'):
                sql = "INSERT INTO timeSeries (ci_Respiratory_Rate_u, ci_Heart_Rate_u, ci_Oxygen_Saturation_u, ci_PLAT_u, ci_NEUT_u, ci_LDH, ci_PO2, ci_CREAT, ci_sa_complications) VALUES (?,?,?,?,?,?,?,?,?)"
                ...
                value_lhd = str(inner_item.get('LDH'))
                ...
                print(sql, value_respiratory_rate_u, value_heart_rate_u, value_oxygen_saturation_u, value_platelets_u, value_neutrophils_u, value_lhd, value_po2, value_creatinine, value_vascular_complication)
                cursor.execute(sql, (value_respiratory_rate_u, value_heart_rate_u, value_oxygen_saturation_u, value_platelets_u, value_neutrophils_u, value_lhd, value_po2, value_creatinine, value_vascular_complication,))
                conn.commit()
